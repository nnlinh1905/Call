...
const incomingCallHourReport = () => {
  ...
  const data = useSelector(dataIncomingHour);
  console.log("nnnnnnnnnnnnnnnnnnnnnnnnn", data);
  const DataIncomingCallHour = {
    chart: {
      type: "column",
    },
    title: {
      text: "Thống kê cuộc gọi đến theo giờ",
      style: {
        color: "#374151",
        fontSize: "15px",
        fontWeight: "600",
    ...
    xAxis: {
      categories: data?.arrayTime ? data?.arrayTime : [],
      crosshair: true,
    },
    yAxis: {
      min: 0,
    ...
    series: [
      {
        name: "Đã trả lời",
        data: data?.arrayValueAnswer ? data?.arrayValueAnswer : [],
        stack: "call",
        color: "#008ffb",
      },
      {
        name: "Gọi nhỡ",
        data: data?.arrayValueMiss ? data?.arrayValueMiss : [],
        stack: "call",
        color: "#00e396",
      },
      {
        name: "Bận",
        data: data?.arrayValueBusy ? data?.arrayValueBusy : [],
        stack: "call",
        color: "#feb019",
      },
      {
        name: "Hủy",
        data: data?.arrayValueCancel ? data?.arrayValueCancel : [],
        stack: "call",
        color: "#ff4560",
      },
      {
        name: "Không trả lời",
        data: data?.arrayValueNoAnswer ? data?.arrayValueNoAnswer : [],
        stack: "call",
        color: "#775dd0",
      },
    ],
  ...
  useEffect(() => {
    dispatch(incomingCallHourReportAPI(request));
  ...
  return (
    <>
      <div className="p-2 bg-[#f2f4f6] mt-2 rounded-2xl shadow">
        <div className="bg-white border w-full rounded-2xl shadow p-2 grid grid-cols-4 items-center gap-4">
          <div className="col-span-4 w-full border-b pb-2 font-medium">
...
export default incomingCallHourReport;
