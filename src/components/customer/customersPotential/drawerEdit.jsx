...
const drawerEdit = (props) => {
  ...
  console.log("sadads", tmp);
  useEffect(() => {
    setemail(tmp?.email);
    setphone(tmp?.phone);
    setaddress(tmp?.address);
    setdistricts_id(tmp?.districts_id);
    setprovince_id(tmp?.province_id);
    setnation_id(tmp?.nation_id);
    setsource_id(tmp?.source_id);
    setname(tmp?.name);
    setwebsite(tmp?.websize);
    setcompany(tmp?.company);
    setpostal_code(tmp?.postal_code);
    setposition(tmp?.position);
    setcustomer_potential_status_id(tmp?.customer_potential_status_id);
    setemployee_charge_id(tmp?.employee_charge_id);
  ...
  const handleSave = () => {
    console.log(params);
    dispatch(editCustomerPotentialAPI(params));
    setIsOpen(false);
    setTimeout(() => {
            ...
            <div className="col-span-1 w-full">
              ...
              <Select
                value={
                  listUser &&
                  listUser.length > 0 &&
                  listUser.filter((item) => item.value == employee_charge_id)
                }
                options={listUser}
                className="!border-gray-50 bg-white text-gray-900 shadow-lg shadow-gray-900/5 ring-4 ring-transparent placeholder:text-gray-500 placeholder:text-xs focus:!border-blue-900 focus:!border-t-blue-900"
          ...
          <article className="relative w-screen flex flex-col overflow-y-scroll h-full max-w-2xl">
            <div className="flex justify-between items-center">
              <div className="p-2 px-10 font-semibold text-lg bg-[#f5f6fa] rounded-t-xl">
                <div className="">Sửa khách hàng tiềm năng</div>
              </div>
            ...
            >
              <div className="text-slate-500 text-sm leading-relaxed grid grid-cols-3 gap-5 justify-center">
                <div className="col-span-1 w-full">
            ...
            <div className="fixed bottom-0 h-12 border-t z-[10000] border-gray-400 flex justify-center items-center gap-4 shadow-xl w-full bg-white rounded-t-lg">
              <Button
                variant="outlined"
                color="error"
                size="small"
