...
const drawerEditUser = (props) => {
  ...
  const dataUser = useSelector(dataShowUser);
  // useEffect(() => {
  //   setIsOpenEdit(true);
  ...
  useEffect(() => {
    ...
    setExtentsion(dataUser?.extension);
    setStatusId(dataUser?.status_id);
    setCode(dataUser?.code);
    setGenderId(dataUser?.gender_id);
    setDepartmentId(dataUser?.department_id);
    setBirthday(dataUser?.employess_detail?.birthday);
    setInfoMore(dataUser?.info_more);
    setEducationId(dataUser?.education_id);
    setPositionId(dataUser?.position_id);
    setId(dataUser?.id);
  }, [dataUser]);
  const sex = [
    { value: 1, label: "Nam" },
  ...
  return (
    <>
      {/* <header className="flex justify-end">
        <Button onClick={() => setIsOpenEdit(true)} variant="outlined">
          ...
          <article className="relative w-screen flex flex-col overflow-y-scroll h-full max-w-2xl">
            <div className="flex justify-between items-center">
              <div className="p-2 px-10 font-semibold text-lg bg-[#f5f6fa] rounded-t-xl">
                <div className="">Sửa thông tin nhân viên</div>
              </div>
